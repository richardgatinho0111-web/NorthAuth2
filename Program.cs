...
// Cloud free hosting (Render/Railway): bind to PORT
...
app.MapGet("/api/v1/admin/music/search", async (HttpRequest req, string? q) =>
{
    ...
    var key = !string.IsNullOrWhiteSpace(settings.YouTubeApiKey) ? settings.YouTubeApiKey.Trim() : envKey?.Trim();
    if (string.IsNullOrWhiteSpace(key))
        return Results.BadRequest(new { success = false, code = "youtube_api_key_missing", message = "Configure a YouTube Data API v3 em Configurações ou na variável YOUTUBE_API_KEY." });

    try
    {
        using var client = new HttpClient { Timeout = TimeSpan.FromSeconds(10) };
        var url = "https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&videoEmbeddable=true&maxResults=1&q="
            + Uri.EscapeDataString(query)
            + "&fields=items(id/videoId,snippet(title,channelTitle,thumbnails/high/url))&key="
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            try
            {
                ...
                var errors = errorDoc.RootElement.GetProperty("error");
                googleMessage = errors.TryGetProperty("message", out var gm) ? gm.GetString() : null;
                if (errors.TryGetProperty("errors", out var details) && details.ValueKind == JsonValueKind.Array)
                {
                    var first = details.EnumerateArray().FirstOrDefault();
                    if (first.ValueKind != JsonValueKind.Undefined && first.TryGetProperty("reason", out var rr))
                        reason = rr.GetString();
                }
            }
            catch { }
            ...
            var friendly = reason switch
            {
                "keyInvalid" => "A chave do YouTube é inválida. Gere uma nova chave no Google Cloud.",
                "accessNotConfigured" => "A YouTube Data API v3 não está habilitada neste projeto do Google Cloud.",
                "quotaExceeded" => "A cota da YouTube Data API v3 foi excedida. Aguarde a renovação da cota ou verifique o projeto.",
                "dailyLimitExceeded" => "O limite diário da YouTube Data API v3 foi excedido.",
                "ipRefererBlocked" => "A chave do YouTube está bloqueada pela restrição de aplicativo configurada no Google Cloud.",
                "forbidden" => "O Google recusou a chave. Confira as restrições da chave e a YouTube Data API v3.",
                _ => "O Google recusou a pesquisa do YouTube. Confira a chave, a API habilitada e as restrições."
            };

            AddLog(req.HttpContext, "YouTube Music API Error", reason ?? "unknown",
                $"HTTP={(int)response.StatusCode}; Query={query}; Google={googleMessage ?? "sem detalhes"}", me.Username);
        ...
        if (!doc.RootElement.TryGetProperty("items", out var items) || items.ValueKind != JsonValueKind.Array)
            return Results.Json(new { success = false, code = "youtube_invalid_response", message = "O YouTube retornou uma resposta inesperada." }, statusCode: 502);

        var item = items.EnumerateArray().FirstOrDefault();
        if (item.ValueKind == JsonValueKind.Undefined ||
            !item.TryGetProperty("id", out var id) ||
            !id.TryGetProperty("videoId", out var videoId))
            return Results.NotFound(new { success = false, message = "Nenhum vídeo compatível encontrado." });

        var snippet = item.GetProperty("snippet");
        var title = snippet.TryGetProperty("title", out var titleEl) ? titleEl.GetString() ?? query : query;
        var channel = snippet.TryGetProperty("channelTitle", out var channelEl) ? channelEl.GetString() ?? "YouTube" : "YouTube";
        ...
        AddLog(req.HttpContext, "YouTube Music Search", title, "Busca realizada no painel", me.Username);
        return Results.Ok(new { success = true, videoId = videoId.GetString(), title, channel, thumbnail });
    }
    catch (HttpRequestException ex)
    {
        AddLog(req.HttpContext, "YouTube Music API Error", "network", $"Falha de rede ao consultar o YouTube: {ex.Message}", me.Username);
        return Results.Json(new { success = false, code = "youtube_network_error", message = "Não foi possível conectar ao YouTube. Verifique sua internet e tente novamente." }, statusCode: 502);
    }
    catch (TaskCanceledException)
    {
        AddLog(req.HttpContext, "YouTube Music API Error", "timeout", "Tempo limite ao consultar o YouTube.", me.Username);
        return Results.Json(new { success = false, code = "youtube_timeout", message = "O YouTube demorou para responder. Tente novamente." }, statusCode: 504);
    }
    catch (JsonException)
    {
        AddLog(req.HttpContext, "YouTube Music API Error", "invalid_json", "Resposta inválida recebida do YouTube.", me.Username);
...
app.MapPost("/api/v1/admin/license/create", (AdminLicenseRequest req, HttpContext ctx) => {
    if(PanelAuthorized(ctx.Request)) { if(!HasPanelPermission(ctx.Request,"licenses")) return Results.StatusCode(403); } else if(!Authorized(ctx.Request,req.ApiKey)) return Results.Unauthorized(); if(string.IsNullOrWhiteSpace(req.License)) return Results.BadRequest(new {success=false,message="License obrigatoria."});
...
public static class PanelPermissions { public static readonly string[] All={"dashboard","applications","licenses","users","logs","settings","sales","webhooks","events","subscriptions","files","variables","rules","tokens","chat","notifications","backups","monitoring","alerts"}; public static readonly string[] StaffDefault={"dashboard","licenses","logs"}; }
